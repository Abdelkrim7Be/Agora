...
call is authorized against explicit policy, human approval is required for
anything hard to undo, and a tamper-evident audit log records every action
taken.
...
- **Not a managed/hosted service.** Self-hosted only: you run the containers
  and hold the LLM and mailbox credentials.
...
The gateway is the only path into an agent; nothing else is reachable
directly from a browser or the internet. Full request flow, agent execution
flow, and trust boundaries: [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).
...
Every layer above is enforced server-side and independently, so a gap in one
