...
def _seed(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(campaigns_module, "DEFAULT_CAMPAIGNS_PATH", path)
    return path
...
def test_loader_roundtrip(tmp_path, monkeypatch):
    ...
    assert cfg.groups[0].members[0].fields["dept"] == "HR"
...
def test_campaign_prepare_and_approve_dry_run(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        # AGENT_DRY_RUN defaults true in the test env → no real send.
        assert result["dry_run"] is True
        assert len(result["sent"]) == 2
        assert result["denied"] == [] and result["failed"] == []

        # Campaign consumed after approval.
