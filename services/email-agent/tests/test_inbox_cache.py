...
def _clear_cache():
    inbox_router._inbox_cache_clear()
    yield
    inbox_router._inbox_cache_clear()
...
def gmail_calls(monkeypatch):
    ...
    patch_provider(
        monkeypatch, inbox_router, list_inbox=_list_inbox, mark_as_read=lambda msg_id: None
    )
    monkeypatch.setattr(inbox_router, "list_runs", lambda *a, **kw: [])
    return calls
...
def test_force_agent_marks_unread_clears_run_and_processes(monkeypatch):
    ...
    patch_provider(monkeypatch, inbox_router, mark_as_unread=lambda msg_id: marked.append(msg_id))
    monkeypatch.setattr(inbox_router, "find_run_by_email", lambda *a, **kw: {"run_id": "run-old"})
    monkeypatch.setattr(inbox_router, "delete_runs", lambda run_ids, **kw: deleted.append(run_ids) or len(run_ids))
    monkeypatch.setattr(inbox_router, "process_message_with_retry", _process)
...
def test_a_zero_ttl_disables_caching(gmail_calls, monkeypatch):
    monkeypatch.setattr(inbox_router, "_INBOX_CACHE_TTL_SECONDS", 0)
...
def test_an_expired_listing_is_served_immediately_instead_of_blocking(monkeypatch):
    ...
    monkeypatch.setattr(inbox_router, "_INBOX_CACHE", {})
    monkeypatch.setattr(inbox_router, "_INBOX_CACHE_TTL_SECONDS", 60.0)
    monkeypatch.setattr(inbox_router, "_INBOX_STALE_SECONDS", 900.0)
    # No shared backend in this test: exercise the local half.
    monkeypatch.setattr(inbox_router, "cache_get_json", lambda _key: None)
    monkeypatch.setattr(inbox_router, "cache_set_json", lambda *a, **k: False)

    inbox_router._inbox_cache_put(key, [{"id": "m1"}])

    fresh, stale = inbox_router._inbox_cache_get(key)
    assert fresh == [{"id": "m1"}] and stale is False

    # Age it past the TTL but inside the stale window.
    stored_at, messages = inbox_router._INBOX_CACHE[key]
    inbox_router._INBOX_CACHE[key] = (stored_at - 120, messages)
    served, stale = inbox_router._inbox_cache_get(key)
    assert served == [{"id": "m1"}]
    ...
    inbox_router._INBOX_CACHE[key] = (_time.time() - 5000, messages)
    assert inbox_router._inbox_cache_get(key) == (None, False)


def test_only_one_background_refresh_runs_per_listing(monkeypatch):
    key = ("someone", "an-instance", "inbox", 25)
    monkeypatch.setattr(inbox_router, "_INBOX_REFRESHING", set())
    scheduled = []
    monkeypatch.setattr(inbox_router.asyncio, "create_task", lambda coro: scheduled.append(coro) or coro.close())

    inbox_router._schedule_inbox_refresh(key, "inbox", 25)
    inbox_router._schedule_inbox_refresh(key, "inbox", 25)
