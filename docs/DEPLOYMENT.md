...
this profile — no change to `email-agent`'s own code.
...
This whole-database dump is the one canonical backup/restore strategy —
it already covers the email-agent's own Alembic-managed tables as a subset,
