...
template: it's grounded in what Agora actually logs and what an operator can
...
### Auth bypass
1. Rotate `GATEWAY_JWT_SECRET`: it forces re-login for every session, which is
...
_(none yet; append dated entries here as they occur)_
